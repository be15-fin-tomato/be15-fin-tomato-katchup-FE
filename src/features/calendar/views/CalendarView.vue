...
const pipelineDailySchedule = computed(() =>
    pipelineEvents.value.filter((event) => event.start === selectedDate.value),
);
...
function goToPipelineDetail(event) {
    ...
    const stepRouteMap = {
        2: { name: 'ListupDetail', paramKey: 'listupId' },
        3: { name: 'ProposalDetail', paramKey: 'proposalId' },
        4: { name: 'QuotationDetail', paramKey: 'quotationId' },
        6: { name: 'ContractDetail', paramKey: 'contractId' },
        7: { name: 'RevenueDetail', paramKey: 'revenueId' },
    };

    const routeInfo = stepRouteMap[stepId];
    if (!routeInfo) {
    ...
    router.push({
        name: routeInfo.name,
        params: { [routeInfo.paramKey]: pipelineId },
    });
}
</script>
        ...
        <div class="flex p-6 gap-6">
            ...
            <div class="w-1/3 bg-white rounded-xl shadow-md p-5 min-h-[400px]">
                ...
                <div class="flex justify-center mt-6">
                    <button class="cursor-pointer" @click="openAddModal">
