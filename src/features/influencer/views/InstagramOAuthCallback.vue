...
const message = ref('인스타그램 계정 연동 처리 중...');
...
onMounted(() => {
  ...
  if (statusParam === 'success') {
    message.value = '인스타그램 계정 연동이 성공적으로 완료되었습니다!';
    error.value = false;
    setTimeout(() => {
    ...
    const decodedErrorMessage = errorMessageParam ?
      ...
      '알 수 없는 오류가 발생했습니다.';
    message.value = `인스타그램 계정 연동 실패: ${decodedErrorMessage}`;
    error.value = true;
  } else {
    message.value = '인스타그램 계정 연동 상태를 알 수 없습니다. 다시 시도해주세요.';
    error.value = true;
  }
});
</script>
