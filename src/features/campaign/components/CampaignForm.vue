...
const selectedCategoryNames = computed(() => {
    ...
    return categories.filter((c) => form.categoryList.includes(c.id)).map((c) => c.name);
});
...
const groups = [
    ...
    },
    {
        ...
        fields: [
            ...
            { key: 'notes', label: '피드백', type: 'textarea', required: false, readOnly: false },
        ],
    },
];
...
const requiredFieldMap = new Map(
    groups.flatMap((group) =>
        group.fields.filter((field) => field.required).map((field) => [field.key, field.label]),
    ),
);
                ...
                <template v-for="field in group.fields" :key="field.key">
                    <div :class="field.width || 'flex-1'">
                        ...
                        <input
                            ...
                            :class="[
                                'input-form-box',
                                isEditing && field.readOnly ? 'bg-gray-100' : 'bg-white',
                            ]"
                        />
