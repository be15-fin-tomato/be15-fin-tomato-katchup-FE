<template>
  <div class="fixed inset-0 bg-black/30 flex justify-center items-center z-50">
    <div class="bg-white rounded-xl p-6 w-[554px] h-[720px] overflow-y-auto font-display">
      <h2 class="text-lg font-semibold mb-6">인플루언서 {{ initialData ? '수정' : '등록' }}</h2>
      ...
      <div class="mb-4">
        <label class="text-sm font-medium mb-1 block">이메일 <span class="text-red-500">*</span></label>
        <input
          type="email"
          class="w-full h-[44px] border border-gray-medium rounded px-3 text-sm"
          v-model="form.email"
      ...
      <div class="grid grid-cols-2 gap-x-4 gap-y-4 mb-6">
        <select class="h-[44px] border border-gray-medium rounded px-3 text-sm" v-model="form.genderDisplay">
        ...
        <select class="h-[44px] border border-gray-medium rounded px-3 text-sm" v-model="form.national"> <option disabled value="">주요 지역</option>
          <option>국내</option>
          <option>국외</option>
        </select>
...
const handleSave = async () => {
  if (!form.value.name || !form.value.email || !form.value.genderDisplay || !form.value.national || form.value.price === '') {
  ...
  const categoryIds = selectedTags.value.map(tagName => {
