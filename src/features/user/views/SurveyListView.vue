    ...
    <div class="w-full min-h-screen bg-background flex">
        ...
        <div class="flex flex-col flex-1 container bg-white">
            ...
            <table class="w-full text-sm text-center border-t mt-10 border-gray-400 table-fixed">
                ...
                >
                    <td class="h-12 px-2 py-0 leading-[3rem] truncate" :title="item.clientManagerName">{{ item.clientManagerName || '-' }}</td>
                    <td class="h-12 px-2 py-0 leading-[3rem] truncate" :title="item.clientCompanyName">{{ item.clientCompanyName || '-' }}</td>
                    <td class="h-12 px-2 py-0 leading-[3rem] truncate" :title="item.campaignName">{{ item.campaignName || '-' }}</td>
                    <td class="h-12 px-2 py-0 leading-[3rem] truncate" :title="item.influencerName">{{ item.influencerName || '-' }}</td>
                    <td class="h-12 px-2 py-0 leading-[3rem] truncate">{{ item.sentDate || '-' }}</td>
                    <td class="h-12 px-2 py-0 leading-[3rem] truncate">{{ item.responseDate || '-' }}</td>
                    <td
                        class="h-12 px-2 py-0 leading-[3rem] truncate"
                        :class="{
